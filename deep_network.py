...
class deep_dropout_NN(Model):
    ...
    def add_placeholders(self):
        ...
        self.y = tf.placeholder(tf.float32,[self.Nbatch,self.Nout],name='y')
    ...
    def add_prediction_op(self):
        ...
        """
        #normalize each input row to be a unit vector.  
        ...
        H1 = fully_connected(inputs=X2,num_outputs=self.Nhidden,
               ...
               weights_regularizer=l2_regularizer(scale=self.alpha2),
               biases_regularizer=l2_regularizer(scale=self.alpha2))
        ...
        outputs=fully_connected(inputs=H3_d,num_outputs=self.Nout,
                                activation_fn=None)
    ...
    def get_batch(self,Xi,yi):
        ...
        ind_sub=np.append(Tvec[:self.Nbatch],Cvec[:self.Nbatch])
        Xsub = Xi[ind_sub]
        y_sub = yi[ind_sub].reshape((len(ind_sub),1))
        # if ( np.sum(y_sub!=y_sub)>0):
        #      print('NaN located in y!?')
        #      raise Exception
